...
def concatenate(arrays, axis=0, compressed_axes=None):
    ...
    if compressed_axes is None: compressed_axes = (axis,)
    if arrays[0].ndim == 1:
    ...
    return GCXS((data, indices, indptr), shape=tuple(shape), compressed_axes=arrays[0].compressed_axes,
         fill_value=arrays[0].fill_value).change_compressed_axes(compressed_axes)
