...
def _get_reduced_shape(shape, params):
    ...
    reduced_coords : np.ndarray
        The reduced coordinates.
    """
    return tuple(sh for sh, p in zip(shape, params, strict=True) if p)
...
def _get_expanded_coords_data(coords, data, params, broadcast_shape):
    ...
    first_dim = -1
    expand_shapes = []
    for d, p, sh in zip(range(len(broadcast_shape)), params, broadcast_shape, strict=True):
    ...
    for d, p in zip(range(len(broadcast_shape)), params, strict=True):
        if p:
...
def _get_matching_coords(coords, params):
    ...
    for p_all in zip(*params, strict=True):
...
def broadcast_to(x, shape):
    ...
    diff_nonbroadcast_idx = [a - b for a, b in zip(nonbroadcast_idx[1:], nonbroadcast_idx[:-1], strict=True)]
...
class _Elemwise:
    ...
    def _get_func_coords_data(self, mask):
        ...
        matched_args = [arg for arg, m in zip(self.args, mask, strict=True) if m is not None and m]
        unmatched_args = [arg for arg, m in zip(self.args, mask, strict=True) if m is not None and not m]
        ...
        m_arg = 0
        for arg, m in zip(self.args, mask, strict=True):
            if m is None:
    ...
    def _match_coo(*args, **kwargs):
        ...
        *args : Tuple[COO]
        ...
        for arg2 in args[1:]:
            ...
            reduced_params = [all(p) for p in zip(*params, strict=True)]
            ...
            linear = [idx[s] for idx, s in zip(linear, sorted_idx, strict=True)]
            matched_idx = _match_arrays(*linear)

            if return_midx:
                return [sidx[midx] for sidx, midx in zip(sorted_idx, matched_idx, strict=True)]

            coords = [arg.coords[:, s] for arg, s in zip(cargs, sorted_idx, strict=True)]
            mcoords = [c[:, idx] for c, idx in zip(coords, matched_idx, strict=True)]
